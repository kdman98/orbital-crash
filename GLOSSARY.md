...
| **Shockwave** | The outward push every reversal emits (repels matter, **pops a Neutral outright** — one reversal, not two). 0.5s cooldown — a tool, not a constant shield. Its radius and force both scale by `1 + hold-charge × 0.5`, so a tap reaches 104px and a fully-loaded reversal reaches 156px. | `P.pulseCd`, `NEUTRAL_POP`, `P.conduitAmp` |
...
| **Ways in** | Two from **matter** — **Volley** (3/body, the main line) and **Ring grind** (0.5/body, for carrying a loaded ring into it) — plus the **baited charge** (4, matter you did not gather but did aim) and two that use no matter at all: **Close reversal** (1, but you must be inside the burst of a thing that does 34 on contact) and **Collapse** (flat 15%). The grind is the only one that needs neither a flip nor a Capacitor, which makes it the fallback when you have been stripped of everything else. *(The Bomber blast went with the payload, and the Fling and Brute barge were removed as damage channels — see the two-channel rule.)* | — |
...
| **Close reversal** | A *charged* reversal whose Shockwave reaches the Anomaly chips it for **1**, no matter needed. The band is narrow: the chip is gated at `hc>=0.5`, so the Shockwave on any reversal that can chip at all reaches **130–156px**, and with its 44px body you must be inside **~174–200px** of a thing that touches for **34**, hunts you, and lunges. It exists so a player stripped of Rings still has a way to keep hurting it. Measured at 2 it was **dominant** — alone it killed an Epoch I Anomaly in 26s for 5 HP — so it is priced to top up, never to be the plan. | `CLOSE_REVERSAL_DMG`, in `flip()` |
